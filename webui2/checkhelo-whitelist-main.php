...
elseif ($_POST['frmaction'] == "delete")  {
	 ...
	 if (isset($_POST['id'])) {
		...
		$db->beginTransaction();
		$res = $db->exec("DELETE FROM ${DB_TABLE_PREFIX}checkhelo_whitelist WHERE ID = ".$db->quote($_POST['id']));	//elimina la policy	
...
elseif ($_POST['frmaction'] == "change")  {
	...
	if (isset($_POST['id'])) {
		...
		if ($_POST['status'] == 1){
			$res = $db->exec("UPDATE ${DB_TABLE_PREFIX}checkhelo_whitelist SET Disabled = 0 WHERE ID = ".$db->quote($_POST['id']));
		}
		ELSE {
			$res = $db->exec("UPDATE ${DB_TABLE_PREFIX}checkhelo_whitelist SET Disabled = 1 WHERE ID = ".$db->quote($_POST['id']));
		}
...
?>
			<tr class="resultsitem">
				...
				<td class="textcenter">
					<form method="post" action="checkhelo-whitelist-main.php">
						<div>
...
?>
	...
	<form method="post" action="checkhelo-whitelist-main.php">
		...
		<table class="entry">
		<caption>Add new Whitelist rule</caption>
			<tr>
				<td class="entrytitle">
					Source
